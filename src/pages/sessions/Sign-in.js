    ...
    <PublicLayout bgImg={`${window.origin}/content/login-page.jpg`}>
      ...
      <Form form={form} layout='vertical' className='mb-4'>
        <Item name='login' rules={[{ required: true, message: <></> }]}>
          <Input value={email} placeholder='Login' onChange={(event)=>setEmail(event.target.value)} />
        </Item>
        ...
        <Button block={false} type='primary' onClick={onSubmit} 
        htmlType='submit' icon={<LoginOutlined style={{ fontSize: '1.3rem' }} />}>
          Login
        </Button>
      </Form>
      <br />
      <p className='mb-1'> <a href='#'>Forgot password?</a></p>
